import { useMemo, useRef } from "react";
import { useFrame } from "@react-three/fiber";
import * as THREE from "three";

function makeGlowTexture(size: number): THREE.CanvasTexture {
  const canvas = document.createElement("canvas");
  canvas.width = size;
  canvas.height = size;
  const ctx = canvas.getContext("2d")!;
  const cx = size / 2;
  const cy = size / 2;
  const r = size / 2;
  const gradient = ctx.createRadialGradient(cx, cy, 0, cx, cy, r);
  gradient.addColorStop(0, "rgba(255, 220, 140, 1)");
  gradient.addColorStop(0.1, "rgba(255, 180, 80, 0.9)");
  gradient.addColorStop(0.3, "rgba(255, 120, 40, 0.5)");
  gradient.addColorStop(0.5, "rgba(255, 80, 20, 0.2)");
  gradient.addColorStop(0.7, "rgba(200, 50, 10, 0.08)");
  gradient.addColorStop(1, "rgba(0, 0, 0, 0)");
  ctx.fillStyle = gradient;
  ctx.fillRect(0, 0, size, size);
  const tex = new THREE.CanvasTexture(canvas);
  tex.needsUpdate = true;
  return tex;
}

function makeCoronaTexture(size: number): THREE.CanvasTexture {
  const canvas = document.createElement("canvas");
  canvas.width = size;
  canvas.height = size;
  const ctx = canvas.getContext("2d")!;
  const cx = size / 2;
  const cy = size / 2;
  const r = size / 2;
  const gradient = ctx.createRadialGradient(cx, cy, 0, cx, cy, r);
  gradient.addColorStop(0, "rgba(255, 255, 255, 0)");
  gradient.addColorStop(0.02, "rgba(255, 200, 100, 0.3)");
  gradient.addColorStop(0.15, "rgba(255, 160, 60, 0.15)");
  gradient.addColorStop(0.4, "rgba(255, 100, 30, 0.06)");
  gradient.addColorStop(0.7, "rgba(200, 50, 10, 0.02)");
  gradient.addColorStop(1, "rgba(0, 0, 0, 0)");
  ctx.fillStyle = gradient;
  ctx.fillRect(0, 0, size, size);
  const tex = new THREE.CanvasTexture(canvas);
  tex.needsUpdate = true;
  return tex;
}

export default function SunGlow() {
  const glowRef = useRef<THREE.Sprite>(null);
  const coronaRef = useRef<THREE.Sprite>(null);

  const glowTex = useMemo(() => makeGlowTexture(256), []);
  const coronaTex = useMemo(() => makeCoronaTexture(512), []);

  useFrame(({ clock, invalidate }) => {
    if (coronaRef.current) {
      const pulse = 1 + 0.04 * Math.sin(clock.elapsedTime * 0.8);
      coronaRef.current.scale.setScalar(70 * pulse);
      coronaRef.current.material.opacity = 0.35 + 0.08 * Math.sin(clock.elapsedTime * 0.6);
    }
    if (glowRef.current) {
      const pulse = 1 + 0.02 * Math.sin(clock.elapsedTime * 1.1);
      glowRef.current.scale.setScalar(25 * pulse);
    }
    invalidate();
  });

  return (
    <>
      <sprite ref={coronaRef} scale={[70, 70, 1]} position={[0, 0, 0]}>
        <spriteMaterial
          map={coronaTex}
          blending={THREE.AdditiveBlending}
          depthWrite={false}
          transparent
          opacity={0.35}
        />
      </sprite>
      <sprite ref={glowRef} scale={[25, 25, 1]} position={[0, 0, 0]}>
        <spriteMaterial
          map={glowTex}
          blending={THREE.AdditiveBlending}
          depthWrite={false}
          transparent
          opacity={0.9}
        />
      </sprite>
    </>
  );
}
