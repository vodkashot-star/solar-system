...
- **Chunk splitting**: `vendor_react` (react/react-dom/scheduler), `vendor_three_core`/`vendor_three_addons`/`vendor_three_stdlib`/`vendor_three_react` (three + R3F), `vendor_fx` (postprocessing), `vendor_animation` (maath/react-spring), `vendor_state` (zustand), `vendor_xr` (**on-demand** — the whole @react-three/xr stack + transitive FontAwesome/devui/styled-components ≈ 5 MB, only pulled in by the lazy AR page; excluded from workbox precache via `globIgnores`), `vendor_shared` (everything else). `preloadThreeChunk` Vite plugin injects `<link rel="modulepreload">` for the hashed `vendor_three_*` assets into built `index.html`
- **AR pages** (`client/src/components/solar-system/ar/`): `#/ar/orrery` (miniature solar system: 8 planets + 9 moons, InstancedMeshes, procedural-noise sun shader, orbit rings) and `#/ar/<bodyId>` (single body GLB + Keplerian moons). `ARCanvas` runs `frameloop="always"` (WebXR needs the loop — the `frameloop="demand"` rule applies only to the main tour Canvas). Placement: `useXRHitTest(cb, 'viewer')` + `<PointerEvents />` + R3F `onClick` on an invisible reticle disc (PointerEvents is null-rendering — it does NOT take children/handlers). Session lifecycle: `createXRStore({ offerSession: false, emulate: false })` + `<XR store={...}>`; enter via `store.enterAR()`, exit via `store.getState().session?.end()`; `useXR((s) => s.mode === "immersive-ar")` inside the Canvas, `usePresenting()` (useSyncExternalStore on the store) in DOM overlays. iOS/no-WebXR → `<model-viewer>` with `ios-src` USDZ (AR Quick Look)
...
- **Production host is Render** (`render.yaml`): 3 services — Express web `node dist/index-prod.js`, FastAPI ML (`run.py serve`), Telegram bot worker. Live: https://solar-system-api-ohxd.onrender.com (ML: https://solar-system-ml.onrender.com). Env: `ALLOWED_ORIGIN` → Render URL, `SPACEAI_URL` → ML service URL (Render services are separate — `localhost` never reaches them), bot worker uses `SOLARIS_API_URL` (Render services are separate — `localhost:5000` wouldn't reach the web service). Blueprint services have secrets as `sync: false` — paste `DATABASE_URL`/tokens in the dashboard after any sync
- **CI (GitHub Actions, `.github/workflows/`)**: `validate.yml` runs the gate — `models:validate` + `typecheck` + `vitest` + `ai:test` (every push); `validate-data.yml` (taxonomy + ML train + tests) and `deploy.yml` (Cloudflare Pages) trigger on branch **`main`**; `deploy.yml` also has `workflow_dispatch`
- **Sentry**: sourcemap upload only when `SENTRY_AUTH_TOKEN` set + `NODE_ENV=production` (`SENTRY_ORG`/`SENTRY_PROJECT`/`SENTRY_RELEASE`)
- **Docs**: completed one-off improvement reports and stale plans live in `docs/archive/` (ARCHITECTURE_IMPROVEMENTS, MODEL_OPTIMIZATION_AUDIT, PERFORMANCE_AUDIT, PWA_IMPLEMENTATION, RESPONSIVE_IMPROVEMENTS, WEBGL_PERFORMANCE_IMPROVEMENTS, PLAN_CONTEXT, old `thoughts/`). Living docs: README.md (features + mermaid architecture), AGENTS.md, CHANGELOG.md, spaceAI/README.md. New one-off reports should be added to `docs/archive/`, not the root
...
- **USDZ for iOS AR**: `client/public/models-usdz/<name>.usdz` (29 files, all < 14 MB) — converted by `scripts/export_usdz.mjs` (`npm run models:usdz` → `convert_usdz.sh`). Pipeline: gltf-transform decompress (Draco+WebP) → weld → meshopt simplify (300k-vertex cap) → **sloppy-collapse fallback + manual buffer compaction** (meshopt's `simplify()` stalls on UV-seamed meshes e.g. ceres; `simplifySloppy` returns `[indices, error]` and needs `compactMesh` + per-attribute remap) → three `USDZExporter.parseAsync` (front-side materials only). Post-export size check re-collapses anything > 14 MB. USDZ URLs are derived `glbUrl.replace(/\.glb$/, ".usdz")` — same jsDelivr SHA, so bump the pointers' SHA when new USDZ files land (the SHA must exist on GitHub first)
