...
planets, and 20+ dwarf planets, asteroids, comets, and interstellar objects.
Rendered with high-quality GLB models. Built with React Three Fiber, Drei, and
Three.js.
...
- **29 GLB models** — sun + 8 planets + 20 dwarf planets, asteroids, comets, and
  interstellar objects, loaded via `useGLTF` and auto-normalized to correct
  scale. Per-body loading grid shows individual progress; overlay dismisses when
  all models load or after a 15s timeout.
...
- **Pause / Free Look** — toggle pauses the tour and enables OrbitControls for
  manual exploration
- **Click-to-focus** — click any planet to fly the camera to it; tour pauses
...
- **Body detail modal** — click "Details" on the HUD card to open a full modal
  with physical/orbital/rotation data, AI classification, and similar-body
  navigation (click to focus)
- **AI classification** — FastAPI microservice classifies bodies by type
  (planet/dwarf planet/asteroid/comet/etc.) with confidence scores and feature
  importance, displayed in the HUD and detail modal
...
The path points to the public `/models/` directory where the actual `.glb`
binary files live. In production, ensure all 9 GLB files are deployed under
`public/models/`.

To switch to an external CDN (e.g. Lovable, Cloudflare R2, S3), update the `url`
field in each `.asset.json` — no code changes needed.
...
npm test         # Run 130+ tests (vitest)
npm run downscale # Downscale Earth/Mercury/Mars GLBs for mobile
npm run models:generate # (Re)generate GLBs with real textures from Solar System Scope
...
| Action | Key / UI | |---|---|---| | Pause tour | Click "Pause tour · Free
look" (top-right) | | Resume tour | Click "Resume tour" (top-right) | | Focus
planet | Click any planet body (click-to-focus, tour pauses) | | Scale toggle |
Click "REALISTIC" / "CINEMATIC" button (top-right) | | Free look (paused) |
Click + drag to rotate, scroll to zoom |

The tour state machine cycles through all 29 celestial bodies. Each body gets 5
...
> `CinematicTour`, and `FocusCamera` all do this.
